...
<div class="banner">
	<img src="images/time/banner.jpg" width="980" height="203" />
</div>

<div class="time">
	<p class="when f_l" id="hour"></p>
    <p class="minutes f_l" id="minute"></p>
    <p class="seconds f_l" id="second"></p>
</div>

<div class="goods">
	<c:forEach var="item" items="${lbList}" varStatus="i">
    <div class="f_l">
		<dl <c:if test="${i.index%4 ne 3 }">style=" border-right:1px dashed #cccccc;"</c:if> >
    		<dt><a href="goods?id=${item.products_id}" target="_blank" title="${item.introduce} 分类：${item.classify} 品牌：${item.brand} 型号： ${item.model}"><img src="images/share/${item.picture}"  width="180" height="135" /></a></dt>
        	<dd>
        		<p><a href="goods?id=${item.products_id}" target="_blank"  title="${item.introduce} 分类：${item.classify} 品牌：${item.brand} 型号： ${item.model}">${fn:substring(item.introduce,0,56)}</a></p>
            	<span title="${item.bewrite }">${fn:substring(item.bewrite,0,18)}</span>
            	<i>抢购价：<em>￥${item.price}</em></i>
            	<input type="button" class="buy" value="立即抢购" onclick="javascript:saveSql(${item.products_id },2,1);"  />
        	</dd>
    	</dl>
    </div>
    </c:forEach>
